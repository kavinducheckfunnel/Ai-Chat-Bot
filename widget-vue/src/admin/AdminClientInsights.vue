  ...
  <div class="ac-page">
    <div class="ac-header">
      ...
      <div v-if="client" class="ac-title-block">
        <h1 class="ac-title">{{ client.name }} — Insights</h1>
        <p class="ac-sub">Per-client metric deep dive (parity with tenant Reports)</p>
      </div>
    </div>
    ...
    <!--
      Reuse PortalReports verbatim — it takes :client as a prop and calls
      api.getPortalAnalytics(client.id) which is the same /api/admin/clients/<id>/analytics/
      endpoint super admin already has access to. No duplication needed.
    -->
    <PortalReports v-else-if="client" :client="client" />
  </div>
</template>
